...
/***/ function(module, exports, __webpack_require__) {
	...
	_handlebars2.default.registerHelper("clusterItems", function (context, options) {
	    var ret = "";
	    ...
	    for (var i = 0; i < context.length; i++) {
	        ...
	        for (var j = 0; j < context[i].items.length; j++) {
	            ...
	            var str = context[i].items[j].url;
	            var target = context[i].id;
	            if (target == "professional_area") {
	                target = "specialization";
	            }

	            var re = new RegExp("" + target + "=\.*?&\\b", "i");
	            var found = str.match(re);

	            ret += "<li class=\"subitem clearfix\"  data-url=" + found + ">" + "<span class=\"subitem__name\">" + context[i].items[j].name + "</span>" + "<span class=\"subitem__count\" >" + context[i].items[j].count + "</span>" + "</li>";
	        }
	        ret += "</ul></li>";
	    }
	...
	var _data = _model2.default.init();

	//отображаем полученные данные
	_data.then(function (data) {
	    localStorage.setItem("newUrl", "");

	    _model2.default.staticData(data);
	    _model2.default.searchResult(data);
	    _model2.default.createFilter(data);

	    checkFilter.addEventListener("click", function (e) {
	        _control2.default.filterCheckClose(e);
	    });

	    searchBtn.addEventListener("click", function () {
	        _control2.default.searchControl();
	    });
	});
...
/***/ function(module, exports, __webpack_require__) {
	...
	var Model = {
	    ...
	    filterInit: function filterInit(url) {
	        return new Promise(function (resolve) {
	            var xhr = new XMLHttpRequest();
	    ...
	    searchResult: function searchResult(data) {
	        var source = _view2.default.searchList();
	        var template = _handlebars2.default.compile(source);
	        var main = template({ items: data.items });
	        document.getElementById("result").innerHTML = main;
	    },
	    //визуализация фильтра вакансий
	    createFilter: function createFilter(data) {
	        var source = _view2.default.filterList();
	        var template = _handlebars2.default.compile(source);
	        var main = template({ clusters: data.clusters });
	        ...
	        filterItems.addEventListener("click", function (e) {
	            e.preventDefault();
	            _control2.default.filterClickControl(e);
	        }, true);
	    },
	    //визуализация выбранного фильтра
	    createFilterLabel: function createFilterLabel(filter, name) {
	    ...
	    staticData: function staticData(data) {
...
/***/ function(module, exports) {
	...
	var View = {
	    ...
	    searchList: function searchList() {
	        var source = "<ul class='search-list'>{{#each items}}<ul class='search-list__items vacancy'>" + "<li class='vacancy__name'>{{name}}</li>" + "{{#if salary.to}}<li class='vacancy__salary'>от {{salary.from}} до {{salary.to}}</li>" + "{{else}}" + "{{#if salary}}<li class='vacancy__salary'>{{salary.from}}</li>{{/if}}" + "{{/if}}" + "<li class='vacancy__description'>{{snippet.requirement}}</li>" + "<li class='employer'>{{employer.name}}</li>" + "{{#if address.city}}<li class='vacancy__town'>{{address.city}}{{/if}} </li> " + "<li>Дата публикации:{{published_at}}</li>" + "</ul>{{/each}}</ul>";
	        return source;
	    },
	    //шаблон фильтра
	    filterList: function filterList() {
	        var source = "<ul class=\'filters\'>{{#clusterItems clusters}}{{/clusterItems}}</ul>";
	        return source;
	    }
	};
...
/***/ function(module, exports, __webpack_require__) {
	...
	var Const = {
	    ...
	    filterClickControl: function filterClickControl(e) {
	        if (e.target.classList.contains("filter")) {
	            var filterSubitems = e.target.querySelector(".filter-subitems");
	            filterSubitems.classList.toggle("visible-f");
	        }

	        if (e.target.closest(".subitem")) {
	            var subitemName = e.target.closest(".subitem");
	            var getUrl = localStorage.getItem("newUrl");
	            var specialization = new RegExp("specialization=\.*?&\\b", "i");
	            ...
	            _model2.default.createFilterLabel(subitemName.getAttribute("data-url"), subitemName.querySelector(".subitem__name").innerHTML);

	            var newData = _model2.default.filterInit(getUrl);
	            newData.then(function (newData) {
	                _model2.default.staticData(newData);
	                _model2.default.createFilter(newData);
	                _model2.default.searchResult(newData);
	            });
	        }
	    },
	    //закрытие лейбл-фильтра
	    filterCheckClose: function filterCheckClose(e) {

	        if (e.target.classList.contains("filter-close")) {

	            var filter = e.target.parentNode.querySelector(".filter-label").getAttribute("data-filter");

	            var url = localStorage.getItem("newUrl");
	            var re = new RegExp("&" + filter + "", "i");
	            var newUrl = url.replace(re, "");

	            checkFilter.removeChild(e.target.parentNode);
	            localStorage.setItem("newUrl", newUrl);
	            var newData = _model2.default.filterInit(newUrl);
	    ...
	    searchControl: function searchControl() {

	        var val = document.querySelector(".search-form__field").value;
	        if (!val) return;
	        var getUrl = void 0;
	        if (localStorage.getItem("newUrl")) {
	        ...
	        _model2.default.createFilterLabel("text=" + val, val);
	        var newData = _model2.default.filterInit(getUrl);
	        newData.then(function (newData) {
	            _model2.default.staticData(newData);
	            _model2.default.createFilter(newData);
